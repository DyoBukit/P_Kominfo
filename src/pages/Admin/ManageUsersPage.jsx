...
function FormTable() {
  ...
  const handleDeleteForm = async (id) => { 
    ...
    try {
      const response = await api.delete(`/admin/evaluasi/${id}`); 
                ...
                <tbody className="bg-transparent divide-y divide-gray-700"> 
                  {forms.map(form => (
                    <tr key={form.id} className="hover:bg-white/5 transition-colors duration-200"> 
                      ...
                      {/* *** BAGIAN PENTING: Mengakses form.user.name *** */}
                      {/* Cek apakah form.user ada sebelum mengakses .name untuk menghindari error jika relasi kosong */}
                      <td className="px-6 py-4 whitespace-nowrap text-gray-100">{form.user ? form.user.name : 'N/A'}</td> 
